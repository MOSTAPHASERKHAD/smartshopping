...
const OUTPUT_SQL = path.join(__dirname, 'import_production.sql');
const OUTPUT_REPORT = path.join(__dirname, 'excluded_data_report.json');
...
let sql = ['-- Migration to Production D1\n'];
let report = {
  summary: {
    total_processed: 0,
    total_excluded: 0,
...
// 1. Settings (No unique constraints to worry about)
if (db.Settings) {
  sql.push('\n-- Settings');
...
// 3. Orders (Must check unique order_id)
if (db.Orders) {
  sql.push('\n-- Orders');
  const seenOrderIds = new Set();
  for (const row of db.Orders) {
    if (seenOrderIds.has(row.order_id)) {
      report.excluded.push({ type: 'Order', reason: 'Duplicate order_id', data: row });
      report.summary.total_excluded++;
      continue;
    }
    seenOrderIds.add(row.order_id);
    report.summary.expected_orders++;
...
// 4. Customers (Must check unique phone)
if (db.Customers) {
  sql.push('\n-- Customers');
  const seenPhones = new Set();
  for (const row of db.Customers) {
    if (seenPhones.has(row.phone)) {
      report.excluded.push({ type: 'Customer', reason: 'Duplicate phone number', data: row });
      report.summary.total_excluded++;
      continue;
    }
    seenPhones.add(row.phone);
    report.summary.expected_customers++;
...
console.log('✅ Audit and SQL generation completed!');
